...
<div class="card">
    <div class="card-header d-flex justify-content-between align-items-center">
        <div>
            <h3 class="mb-0">
                <i class="fas fa-user-plus me-2"></i>إضافة فرد جديد
            </h3>
            @if($family)
                <small class="text-muted">إلى عائلة {{ $family->name_ar ?? 'غير محدد' }}</small>
            @endif
        </div>
        <a href="{{ $family ? route('families.show', $family) : route('persons.index') }}" class="btn btn-outline-primary">
    ...
    <div class="card-body">
        <form action="{{ $family ? route('families.persons.store', $family) : route('persons.store') }}" method="POST">
                ...
                <div class="col-md-12">
                    ...
                    <div class="mb-3">
                        <label class="form-label">
                            اختر العائلة <span class="text-danger">*</span>
                        </label>
                        <select name="family_id" class="form-select @error('family_id') is-invalid @enderror" required>
                            <option value="">-- اختر العائلة --</option>
                            @foreach($recentFamilies as $family)
                                <option value="{{ $family->id }}">
                                    عائلة {{ $family->name_ar ?? 'غير محدد' }}
                ...
                <div class="col-md-3">
                    <div class="mb-3">
                        ...
                        <select name="relationship_to_family_head" id="relationship_to_family_head" class="form-select @error('relationship_to_family_head') is-invalid @enderror" required>
                            ...
                            <option value="رب الأسرة" {{ old('relationship_to_family_head') == 'رب الأسرة' ? 'selected' : '' }}>رب الأسرة</option>
                            <option value="زوجة" {{ old('relationship_to_family_head') == 'زوجة' ? 'selected' : '' }}>زوجة</option>
                            <option value="ابن" {{ old('relationship_to_family_head') == 'ابن' ? 'selected' : '' }}>ابن</option>
                            <option value="ابنة" {{ old('relationship_to_family_head') == 'ابنة' ? 'selected' : '' }}>ابنة</option>
                            <option value="أب" {{ old('relationship_to_family_head') == 'أب' ? 'selected' : '' }}>أب</option>
                            <option value="أم" {{ old('relationship_to_family_head') == 'أم' ? 'selected' : '' }}>أم</option>
                            <option value="أخ" {{ old('relationship_to_family_head') == 'أخ' ? 'selected' : '' }}>أخ</option>
                            <option value="أخت" {{ old('relationship_to_family_head') == 'أخت' ? 'selected' : '' }}>أخت</option>
                            <option value="جد" {{ old('relationship_to_family_head') == 'جد' ? 'selected' : '' }}>جد</option>
                            <option value="جدة" {{ old('relationship_to_family_head') == 'جدة' ? 'selected' : '' }}>جدة</option>
                            <option value="عم" {{ old('relationship_to_family_head') == 'عم' ? 'selected' : '' }}>عم</option>
                            <option value="عمة" {{ old('relationship_to_family_head') == 'عمة' ? 'selected' : '' }}>عمة</option>
                            <option value="خال" {{ old('relationship_to_family_head') == 'خال' ? 'selected' : '' }}>خال</option>
                            <option value="خالة" {{ old('relationship_to_family_head') == 'خالة' ? 'selected' : '' }}>خالة</option>
                            <option value="ابن الأخ" {{ old('relationship_to_family_head') == 'ابن الأخ' ? 'selected' : '' }}>ابن الأخ</option>
                            <option value="ابنة الأخ" {{ old('relationship_to_family_head') == 'ابنة الأخ' ? 'selected' : '' }}>ابنة الأخ</option>
                            <option value="ابن الأخت" {{ old('relationship_to_family_head') == 'ابن الأخت' ? 'selected' : '' }}>ابن الأخت</option>
                            <option value="ابنة الأخت" {{ old('relationship_to_family_head') == 'ابنة الأخت' ? 'selected' : '' }}>ابنة الأخت</option>
                            <option value="حفيد" {{ old('relationship_to_family_head') == 'حفيد' ? 'selected' : '' }}>حفيد</option>
                            <option value="حفيدة" {{ old('relationship_to_family_head') == 'حفيدة' ? 'selected' : '' }}>حفيدة</option>
                            <option value="أخرى" {{ old('relationship_to_family_head') == 'أخرى' ? 'selected' : '' }}>أخرى</option>
                        </select>
                    ...
                    <div class="mb-3">
                        ...
                        <select name="father_id" id="father_id" class="form-select @error('father_id') is-invalid @enderror">
                            ...
                            @foreach($familyMembers as $id => $name)
                                <option value="{{ $id }}" {{ old('father_id') == $id ? 'selected' : '' }}>{{ $name }}</option>
                            @endforeach
                        </select>
                        @error('father_id')
                            <div class="invalid-feedback">{{ $message }}</div>
            ...
            <div class="mt-4 text-center">
                ...
                <a href="{{ $family ? route('families.show', $family) : route('persons.index') }}" class="btn btn-outline-secondary">
